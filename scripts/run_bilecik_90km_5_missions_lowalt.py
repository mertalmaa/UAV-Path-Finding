"""Same 5 ~90 km Bilecik missions as run_bilecik_90km_5_missions.py, but with the
31km single-shot script's low-altitude-cost config applied (config ablation test).

Only the cost-function knobs differ from run_bilecik_90km_5_missions.py, copied
verbatim from scripts/run_single_shot_31km.py:
  enable_low_altitude_cost = True
  low_altitude_cost_shape  = "linear"
  lambda_agl               = 0.25
  agl_cost_scale_m         = 1000.0
  desired_agl_m            = 120.0   (same value 90km used implicitly via default)

Everything else (missions, start/goal, tolerances, ROI, buffers, weights,
pipeline stages) is identical to run_bilecik_90km_5_missions.py so this is a
clean isolation of that one factor.

Usage:
  python -B scripts/run_bilecik_90km_5_missions_lowalt.py            # all 5
  python -B scripts/run_bilecik_90km_5_missions_lowalt.py --mission M90_01
"""
...
BENCHMARK_JSON = RESULTS_DIR / "bilecik_90km_5_missions_lowalt_benchmark.json"
...
def build_config() -> PlannerConfig:
    return dataclasses.replace(
        ...
        # --- 31km single-shot's low-altitude-cost knobs, copied verbatim ---
        enable_low_altitude_cost=True,
        low_altitude_cost_shape="linear",
        lambda_agl=0.25,
        agl_cost_scale_m=1000.0,
...
def plan_and_evaluate_single_shot(
    ...
    print(f"\n{'='*75}")
    print(f"[{mission_id}] {title} — SINGLE-SHOT, LOW-ALTITUDE-COST ON (31km config)")
    ...
    print("  -> Tek Seferde Dual-Queue A* Arama (lambda_agl=0.25, linear) Baslatiliyor...", flush=True)
    ...
    plot_file = PLOTS_DIR / f"{mission_id.lower()}_lowalt_fidelity_report.png"
    ...
    result_data = {
        ...
        "structure_type": spec["structure_type"],
        "description_tr": spec["description_tr"],
        "planning_mode": "SINGLE_SHOT_LOW_ALTITUDE_COST_31KM_CONFIG",
        "success": True,
        "expanded_nodes": search_res.expanded_nodes,
        "search_time_s": round(search_time, 3),
...
def generate_single_shot_4panel_plot(
    ...
    ax1.plot(x_m, y_m, "b-", lw=2.2, label=f"Low-Alt-Cost Path ({smooth_res.total_distance_m/1000:.2f} km)")
    ...
    ax1.set_title(f"1. Full 90x90km DEM (lambda_agl=0.25): {spec['id']} ({spec['structure_type']})", fontsize=11, fontweight="bold")
    ...
    fig.suptitle(f"Bilecik 90x90km LOW-ALT-COST (31km config) — {spec['id']}: {spec['title_tr']}", fontsize=13, fontweight="bold", y=0.98)
...
def save_all(results_by_id: Dict[str, Dict]):
    ...
    payload = {
        "region": "bilecik",
        "roi_size_km": 90.0,
        "config_note": "31km single-shot low-altitude-cost config applied (enable_low_altitude_cost=True, linear, lambda_agl=0.25, agl_cost_scale_m=1000.0)",
...
def main():
    ...
    print("=" * 80)
    print("BILECIK 90x90 KM — 31KM LOW-ALTITUDE-COST CONFIG ABLATION TESTI")
    print("=" * 80)
