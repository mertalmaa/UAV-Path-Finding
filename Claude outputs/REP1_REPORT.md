...
> **CORRECTION (Step REP-1.1, 2026-09-13):** this report describes
> `webapp/server.py` below as "the one live interactive tool" / "the TEK
> gerçek canlı production aracı" -- accurate when this report was written,
> no longer accurate today. The very next stage, REP-1.1, deleted
> `webapp/server.py`, `webapp/static/index.html` and `.claude/launch.json`
> entirely (see `REP11_REPORT.md`). Every `webapp/server.py` reference
> below is kept as-is as the historical record of REP-1's own audit, not
> because that file still exists.
