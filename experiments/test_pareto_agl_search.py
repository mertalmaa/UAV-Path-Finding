...
def test_pareto_agl_search():
    ...
    def edge_cost_fn(tr: PhysicalTrajectory):
        ...
        mu = cost_surf[r, c]
        local_elev = elev_grid[r, c]
        excess_agl = max(0.0, (ep.z_msl_m - local_elev) - 120.0)
        return l3d * (mu + 0.20 * (excess_agl / 500.0))
    ...
    while open_heap and expanded < 10000:
        ...
        for prim, traj in _candidate_trajectories(node.end_pose, envelope, CONFIG):
            ...
            for fg, fm, _ in frontier:
                # Dominated only if another state has lower/equal cost AND lower/equal excess AGL
                if fg <= cand_g + 1e-9 and fm <= cand_metric + 1e-9:
                    dominated = True
                    break
            if dominated:
                rejections["PARETO_AGL_DOM"] += 1
                continue
    ...
    if goal_node:
        ...
        print(f"FAILED in {expanded} expansions. Rejections: {dict(rejections)}")

if __name__ == "__main__":
    test_pareto_agl_search()
