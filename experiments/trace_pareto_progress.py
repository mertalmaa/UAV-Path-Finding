...
while open_heap and expanded < 8000:
    ...
    if expanded % 200 == 0:
        ...
        r, c = terrain.xy_to_rowcol(node.end_pose.x_m, node.end_pose.y_m)
        elev = elev_grid[r, c] if 0<=r<rows and 0<=c<cols else 0.0
        agl = node.end_pose.z_msl_m - elev
        print(f"Exp {expanded:4d}: pos=({node.end_pose.x_m:.0f}, {node.end_pose.y_m:.0f}, z={node.end_pose.z_msl_m:.0f} MSL), AGL={agl:.1f}m, d_xy={d_xy:.0f}m, d_z={abs(node.end_pose.z_msl_m - goal.z_msl_m):.0f}m, OPEN={len(open_heap)}")
