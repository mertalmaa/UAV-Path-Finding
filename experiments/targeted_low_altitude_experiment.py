"""Targeted Investigation of Non-Destructive Low-Altitude Mechanisms for Fixed-Wing Global A*.

Evaluates:
1. Baseline Weighted A* w=1.01 (pure geometric, FIFO)
2. Pareto Goal-Z Dominance (dominance on g and |z - z_goal|)
3. Pareto Lookahead Terrain Envelope Dominance (dominance on g and |z - z_target|)
4. Envelope-Guided Admissible Heuristic (h accounts for vertical climb/descent profile)
5. Integrated Low-Altitude Guidance
"""
...
class VariantConfig:
    ...
    dominance_mode: str = "none" # "none", "goal_z", "envelope_z"
    heuristic_mode: str = "euclidean" # "euclidean", "envelope"
...
def run_targeted_search(
    ...
    if var.dominance_mode != "none":
        start_xyh = (start_key.x_bin, start_key.y_bin, start_key.heading_bin)
        if var.dominance_mode == "goal_z":
            z_metric = abs(start.z_msl_m - goal.z_msl_m)
        else:
            z_targ = compute_target_envelope_alt(start, goal, terrain, var.target_agl_m, var.lookahead_m, var.climb_slope)
            z_metric = max(0.0, start.z_msl_m - z_targ)
        pareto_frontier[start_xyh].append((0.0, z_metric, start_key))

    def eval_heuristic(p: PhysicalPose) -> float:
        if var.heuristic_mode == "euclidean":
            return _heuristic(p, goal, goal_tolerance)
        else:
            # Envelope-aware heuristic:
            dx = max(abs(p.x_m - goal.x_m) - goal_tolerance.xy_m, 0.0)
            dy = max(abs(p.y_m - goal.y_m) - goal_tolerance.xy_m, 0.0)
            d_xy = math.hypot(dx, dy)
            z_targ = compute_target_envelope_alt(p, goal, terrain, var.target_agl_m, var.lookahead_m, var.climb_slope)
            # Vertical climb/descent component
            dz_to_target = max(0.0, abs(p.z_msl_m - z_targ) - goal_tolerance.altitude_m)
            dz_to_goal = max(0.0, abs(p.z_msl_m - goal.z_msl_m) - goal_tolerance.altitude_m)
            dz_eff = max(dz_to_target, dz_to_goal)
            return math.sqrt(d_xy * d_xy + dz_eff * dz_eff)

    h_0 = eval_heuristic(start)
    f_0 = 0.0 + var.heuristic_weight * h_0
        ...
        for primitive, trajectory in _candidate_trajectories(node.end_pose, envelope, config):
            ...
            if var.dominance_mode != "none":
                ...
                if var.dominance_mode == "goal_z":
                    ...
                    z_targ = compute_target_envelope_alt(end_pose, goal, terrain, var.target_agl_m, var.lookahead_m, var.climb_slope)
                    cand_z_metric = max(0.0, end_pose.z_msl_m - z_targ)
            ...
            f_score = candidate_g + var.heuristic_weight * h_score
...
def main():
    print("=" * 95)
    print("TARGETED INVESTIGATION OF NON-DESTRUCTIVE LOW-ALTITUDE MECHANISMS")
    print("=" * 95)
    ...
    variants = [
        ...
        VariantConfig(
            name="Pareto_GoalZ_w101",
            description="Weighted A* w=1.01 + Pareto Goal-Z Dominance",
            heuristic_weight=1.01,
            dominance_mode="goal_z",
        ),
        VariantConfig(
            name="Pareto_EnvelopeZ_w101",
            description="Weighted A* w=1.01 + Pareto Lookahead Envelope Dominance",
            heuristic_weight=1.01,
            dominance_mode="envelope_z",
        ),
    ]
    ...
    out_file = ROOT / "results" / "targeted_low_altitude_experiment.json"
