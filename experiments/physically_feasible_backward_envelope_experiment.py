"""Physically Feasible Backward-Envelope Low-Altitude Global A* Search.

Implements the exact 1D Backward-Pass Dynamic Reachability Formulation:
1. Along the mission baseline (or local heading), computes backward climb constraints from goal and terrain peaks:
   Z_floor(s) = max(terrain(s) + 120m, Z_floor(s+ds) - climb_slope * ds)
2. This guarantees that Z_floor(s) is 100% aerodynamically feasible by construction.
3. Evaluates on all canonical missions A-F.
"""
...
def compute_1d_backward_feasible_envelope(
    ...
    """Compute the 1D backward-reachable terrain-following floor profile."""
...
@dataclass
class EnvelopeSearchConfig:
    name: str
    description: str
    heuristic_weight: float = 1.01
    mode: str = "dominance" # "baseline", "dominance", "tie_break", "cost"
...
def run_envelope_guided_search(
    start: PhysicalPose,
    goal: GoalPose,
    terrain: TerrainQuery,
    aircraft_profile,
    cfg: EnvelopeSearchConfig,
    ...
    # Precompute 1D backward feasible envelope along baseline
    g_dists, g_elevs, g_z_floor = compute_1d_backward_feasible_envelope(
        start, goal, terrain, cfg.target_agl_m, climb_slope=0.125, sample_spacing_m=30.0,
    )
    total_baseline = g_dists[-1]

    def get_feasible_floor(pose: PhysicalPose) -> float:
        # Project pose onto start->goal baseline
        dx = goal.x_m - start.x_m
        dy = goal.y_m - start.y_m
        L2 = dx*dx + dy*dy
        if L2 == 0:
    ...
    def eval_edge_cost(trajectory: PhysicalTrajectory, safety) -> float:
        ...
        if cfg.mode == "cost":
            ...
            z_floor_end = get_feasible_floor(end_p)
            excess = max(0.0, end_p.z_msl_m - z_floor_end)
            return geom_len * (1.0 + cfg.w_alt * (excess / 1000.0))
        return geom_len

    def make_queue_item(f: float, g: float, h: float, pose: PhysicalPose, nid: int):
        c = next(counter)
        if cfg.mode == "tie_break":
            band = round(f / cfg.f_band_m) if cfg.f_band_m > 0 else f
            z_fl = get_feasible_floor(pose)
            excess = max(0.0, pose.z_msl_m - z_fl)
...
def main():
    print("=" * 95)
    print("PHYSICALLY FEASIBLE BACKWARD-ENVELOPE EXPERIMENT (A-F)")
    print("=" * 95)
    ...
    variants = [
        EnvelopeSearchConfig(
            name="Baseline_w101",
            description="Baseline Weighted A* (w=1.01, pure geometric)",
            mode="baseline",
        ),
        EnvelopeSearchConfig(
            name="Feasible_Dominance_d25",
            description="Feasible Backward-Envelope Dominance (delta_g=25m)",
            mode="dominance",
            delta_g_m=25.0,
        ),
        EnvelopeSearchConfig(
            name="Feasible_Cost_w010",
            description="Feasible Backward-Envelope Cost (w_alt=0.10)",
            mode="cost",
            w_alt=0.10,
        ),
        EnvelopeSearchConfig(
            name="Feasible_Cost_w025",
            description="Feasible Backward-Envelope Cost (w_alt=0.25)",
            mode="cost",
            w_alt=0.25,
        ),
        EnvelopeSearchConfig(
            name="Feasible_TieBreak_B25",
            description="Feasible Backward-Envelope F-Band Tie-Break (band=25m)",
            mode="tie_break",
            f_band_m=25.0,
        ),
    ]
    ...
    for var in variants:
        ...
        for m_id, m_data in clean_missions.items():
            ...
            res, met = run_envelope_guided_search(
    ...
    out_file = ROOT / "results" / "physically_feasible_backward_envelope_experiment.json"
